...
function preTableValores() {
   ...
   $('.table').on("click", ".inverteStatus", function (e) {
      var params = {
         idvalores: $(this).attr("codigo")
      }

      $.post(
         'valores_script.php?opx=inverteStatus',
         params,
         function (data) {
...
var myColumnDefs = [
	{ key: "idvalores", sortable: true, label: "ID", print: true, data: true },
		...
		success: function (data) {
			...
			if (data.totalRecords > 0) {
				$.each(data.records, function (index, value) {
					...
					tr += '<a href="index.php?mod=valores&acao=formValores&met=editValores&idu=' + value.idvalores + '"><img src="images/ico_edit.png" height="16" width="16" alt="ico" /><div class="tt"><span class="one">Editar</span><span class="two"></span></div></a>';
					tr += '<a href="#" onclick="wConfirm(\'Excluir Registro\',\'Tem certeza que deseja excluir o registro ' + value.nome + ' ?\',\'php\', \'valores_script.php?opx=deletaValores&idu=' + value.idvalores + '\');"><img src="images/ico_del.png" height="17" width="17" alt="ico" /><div class="tt"><span class="one">Excluir</span><span class="two"></span></div></a>';
					tr += '</div></td>';
				});
...
function verificaUrlrewrite(url) {
	...
	$.ajax({
		url: 'valores_script.php',
		dataType: 'json',
		data: "opx=verificarUrlRewrite&idvalores=" + id + "&urlrewrite=" + url,
		type: 'post',
		beforeSend: function () {
			Loader.show();
		},
...
function addGrid(nome, uploadImagem = true, nomeInput1 = "nome", nomeInput2 = "descricao", width = 96, height = 96){
    var novoinput = '';
    ...
    novoinput += '<input type="text" class="box_txt input'+nomeCapitalized+' w-100" name="'+nome+'['+$('.box-'+nome).length+']['+nomeInput1+']" placeholder="Titulo">';
    novoinput += '<textarea rows="6" type="text" style="resize: vertical" class="box_txt input'+nomeCapitalized+' w-100" name="'+nome+'['+$('.box-'+nome).length+']['+nomeInput2+']" placeholder="Texto"></textarea>';
    novoinput += '</td>';
...
function subirFaq ( el ) {
   ...
   var prevTr = $(thisTr).prevAll('.box-valores_faq').first();
...
function descerFaq( el ) {
   ...
   var auxDiv = $('.div-aux');
   var thisTr = $(el).parent().parent().parent();
   var nextTr = $(thisTr).nextAll('.box-valores_faq').first();
   var thisInputPos = $(thisTr).find('.td-flex').find('input');
...
function excluirFaq (el) {
   ...
   $('.removeValores_faq-'+key).hide();
   // $('.removeFaq-'+key).next('tr').remove();
   $('#excluirValores_faq-'+key).val('0');
   // $('.removeFaq-'+key).remove();
};

function addActionFaq() {
   $(".subirValores_faq").unbind( "click" );
   $(".descerValores_faq").unbind( "click" );
   $(".excluirValores_faq").unbind( "click" );

   $(".subirValores_faq").on("click", function () {
       subirFaq($(this));
   });

   $(".descerValores_faq").on("click", function () {
       descerFaq($(this));
   });

   $(".excluirValores_faq").on("click", function () {
       excluirFaq($(this));
   });
}
...
$(document).ready(function () {
       ...
       $(".btn-valores_faq").on('click', function () {
         addGrid('valores_faq', false, "pergunta", "resposta");
     });
    ...
    $(".excluir-arquivo").click(function(e){
      ...
      $.ajax({
         ...
         type: "POST",
         dataType: "json",
         data: {opx: 'excluirArquivo', idvalores: $("#idvalores").val(), arquivo: $(thisElement).attr('data-arquivo')},
         beforeSend: function () {
            Loader.show();
         },
         success: function (data) {
   ...
   $("#formValores").on("click", ".btn-choose-icon", function(){
   ...
   ////////////////////////////////////////////
      ...
      $(".btExcluirImagem").click(function(e){
         ...
         idvalores = $("#formValores").find("#idvalores").val();
         idvalores_imagem = $("#"+idPosicao).find("input[name='idvalores_imagem[]']").val();
         imagem = $("#"+idPosicao).find("input[name='imagem_valores[]']").val();
         ref = $("#"+idPosicao); 
         ...
         $.ajax({
            ...
            data:
            {
              ...
              idvalores:idvalores,            
              imagem:imagem,
              idvalores_imagem:idvalores_imagem
              // descricao: post2
            },
            beforeSend:function(){
              Loader.show();
   ...
   ////////////////////////////////////////////////
    ...
    $('.save-cropped-image').click(function(e){
        ...
        formData.append("idvalores", $('input[name=idvalores]').val());
        if(dataImageType == 'imagem'){
            ...
            success:function(data){
                ...
                if(data.status == true){
                    if(dataImageType == 'imagem'){
                        ...
                        $('#select-image-1 .img-valores-form').attr('src', 'files/valores/'+data.imagem);
                        ...
                        $('#select-image-2 .img-valores-form').attr('src', 'files/valores/'+data.imagem);
    ...
    //         location.href='index.php?mod=valores&acao=listarValores';
    //     }
    //     else{
    //         if($("#idvalores").val() == 0){
    //             if($("#imagem-value").val() != ''){
    //                 cancelarImagem('valores', $("#imagem-value").val());
    //             }
    ...
    $(".confirm").click(function(){
        excluirImagem('valores', tipo, img, thisElem);
...
function enviaImagensAjax(input, posicao, limite){ 
  ...
  formData.append('idvalores', $("#idvalores").val()); 
  ...
  $.ajax({ 
    ...
    success:function(data){  
        if(data.status == true){ 
            $li = '<li class="ui-state-default'+posicao+' move" id="'+posicao+'" idimagem="'+data.idvalores_imagem+'">';
            $li += '<img id="img'+posicao+'" class="imagem-gallery" src="'+data.caminho+'">';
            $li += '<a class="editImagemDescricao" idimagem="'+data.idvalores_imagem+'" href="#"><button class="edit"></button></a>';
            $li += '<a class="excluirImagem" idimagemdelete="'+data.idvalores_imagem+'" href="#"><button class="delete"></button></a>';
            $li += '<a class="postImagem" idimagempost="'+data.idvalores_imagem+'" href="#"><button class="post_imagem"></button></a>'; 
            $li += '<a class="postImagem" idimagempost="'+data.idvalores_imagem+'" href="#"><button class="post_imagem"></button></a>';
            $li += '<input type="hidden" name="idvalores_imagem[]" value="'+data.idvalores_imagem+'">'; 
            $li += '<input type="hidden" name="descricao_imagem[]" value="">'; 
            $li += '<input type="hidden" name="imagem_valores[]" value="'+data.nome_arquivo+'">';
            $li += '<input type="hidden" name="posicao_imagem[]" value="'+posicao+'">';
            $li += '</li>'; 
            $("#sortable").append($li); 
            $("#idvalores").val(data.idvalores); 
            if(numImagem > 1){
