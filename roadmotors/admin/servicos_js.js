		...
		success: function (data) {
			...
			if (data.totalRecords > 0) {
				$.each(data.records, function (index, value) {
					...
					tr += '<a href="#" onclick="wConfirm(\'Excluir Registro\',\'Tem certeza que deseja excluir o registro ' + value.nome + ' ?\',\'php\', \'servicos_script.php?opx=deletaServicos&idu=' + value.idservicos + '\');"><img src="images/ico_del.png" height="17" width="17" alt="ico" /><div class="tt"><span class="one">Excluir</span><span class="two"></span></div></a>';
					tr += '</div></td>';
				});
...
function subirFaq ( el ) {
   ...
   var prevTr = $(thisTr).prevAll('.box-servicos_faq').first();
...
function descerFaq( el ) {
   ...
   var auxDiv = $('.div-aux');
   var thisTr = $(el).parent().parent().parent();
   var nextTr = $(thisTr).nextAll('.box-servicos_faq').first();
   var thisInputPos = $(thisTr).find('.td-flex').find('input');
...
function excluirFaq (el) {
   ...
   $('.removeServicos_faq-'+key).hide();
   // $('.removeFaq-'+key).next('tr').remove();
   $('#excluirServicos_faq-'+key).val('0');
   // $('.removeFaq-'+key).remove();
};

function addActionFaq() {
   $(".subirServicos_faq").unbind( "click" );
   $(".descerServicos_faq").unbind( "click" );
   $(".excluirServicos_faq").unbind( "click" );

   $(".subirServicos_faq").on("click", function () {
       subirFaq($(this));
   });

   $(".descerServicos_faq").on("click", function () {
       descerFaq($(this));
   });

   $(".excluirServicos_faq").on("click", function () {
       excluirFaq($(this));
   });
}
...
$(document).ready(function () {
       ...
       $(".btn-servicos_faq").on('click', function () {
         addGrid('servicos_faq', false, "pergunta", "resposta");
     });
  ...
  $("#icone_upload3").change(function(){
   ...
   $("#inputArquivoBotao3").find('.btn').val($(this)[0].files[0].name);
    ...
    $(".excluir-arquivo").click(function(e){
      ...
      $.ajax({
         ...
         type: "POST",
         dataType: "json",
         data: {opx: 'excluirArquivo', idservicos: $("#idservicos").val(), arquivo: $(thisElement).attr('data-arquivo')},
         beforeSend: function () {
            Loader.show();
         },
         success: function (data) {
   ...
   $("#formServicos").on("click", ".btn-choose-icon", function(){
   ...
   ////////////////////////////////////////////
      ...
      $(".btExcluirImagem").click(function(e){
         ...
         idservicos = $("#formServicos").find("#idservicos").val();
         idservicos_imagem = $("#"+idPosicao).find("input[name='idservicos_imagem[]']").val();
         imagem = $("#"+idPosicao).find("input[name='imagem_servicos[]']").val();
         ref = $("#"+idPosicao); 
         ...
         $.ajax({
            ...
            data:
            {
              ...
              imagem:imagem,
              idservicos_imagem:idservicos_imagem
              // descricao: post2
            },
            beforeSend:function(){
              Loader.show();
   ...
   ////////////////////////////////////////////////
    ...
    $('.save-cropped-image').click(function(e){
        ...
        formData.append("idservicos", $('input[name=idservicos]').val());
        if(dataImageType == 'imagem'){
            ...
            success:function(data){
                ...
                if(data.status == true){
                    if(dataImageType == 'imagem'){
                        ...
                        $('#select-image-1 .img-servicos-form').attr('src', 'files/servicos/'+data.imagem);
                        ...
                        $('#select-image-2 .img-servicos-form').attr('src', 'files/servicos/'+data.imagem);
    ...
    //         location.href='index.php?mod=servicos&acao=listarServicos';
    //     }
    //     else{
    //         if($("#idservicos").val() == 0){
    //             if($("#imagem-value").val() != ''){
    //                 cancelarImagem('servicos', $("#imagem-value").val());
    //             }
    ...
    $(".confirm").click(function(){
        excluirImagem('servicos', tipo, img, thisElem);
...
function enviaImagensAjax(input, posicao, limite){ 
  ...
  formData.append('idservicos', $("#idservicos").val()); 
  ...
  $.ajax({ 
    ...
    success:function(data){  
        if(data.status == true){ 
            $li = '<li class="ui-state-default'+posicao+' move" id="'+posicao+'" idimagem="'+data.idservicos_imagem+'">';
            $li += '<img id="img'+posicao+'" class="imagem-gallery" src="'+data.caminho+'">';
            $li += '<a class="editImagemDescricao" idimagem="'+data.idservicos_imagem+'" href="#"><button class="edit"></button></a>';
            $li += '<a class="excluirImagem" idimagemdelete="'+data.idservicos_imagem+'" href="#"><button class="delete"></button></a>';
            // $li += '<a class="postImagem" idimagempost="'+data.idservicos_imagem+'" href="#"><button class="post_imagem"></button></a>'; 
            // $li += '<a class="postImagem" idimagempost="'+data.idservicos_imagem+'" href="#"><button class="post_imagem"></button></a>';
            $li += '<input type="hidden" name="idservicos_imagem[]" value="'+data.idservicos_imagem+'">'; 
            $li += '<input type="hidden" name="descricao_imagem[]" value="">'; 
            $li += '<input type="hidden" name="imagem_servicos[]" value="'+data.nome_arquivo+'">';
            $li += '<input type="hidden" name="posicao_imagem[]" value="'+posicao+'">';
            $li += '</li>'; 
            $("#sortable").append($li); 
            $("#idservicos").val(data.idservicos); 
            if(numImagem > 1){
