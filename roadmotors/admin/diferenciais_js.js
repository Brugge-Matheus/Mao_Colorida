...
function preTableDiferenciais() {
	$("#limit").change(function () {
		$("#pagina").val(1);
		dataTableDiferenciais();
	});
   ...
   $('.table').on("click", ".inverteStatus", function (e) {
      var params = {
         iddiferenciais: $(this).attr("codigo")
      }

      $.post(
         'diferenciais_script.php?opx=inverteStatus',
         params,
         function (data) {
...
var myColumnDefs = [
	{ key: "iddiferenciais", sortable: true, label: "ID", print: true, data: true },
	{ key: "nome", sortable: true, label: "Nome", print: true, data: true },
	{ key: "titulo", sortable: true, label: "Titulo", print: true, data: true },
...
function dataTableDiferenciais() {
	limit = $("#limit").val();
	pagina = $("#pagina").val();
		...
		success: function (data) {
			...
			if (data.totalRecords > 0) {
				$.each(data.records, function (index, value) {
					...
					tr += '<a href="index.php?mod=diferenciais&acao=formDiferenciais&met=editDiferenciais&idu=' + value.iddiferenciais + '"><img src="images/ico_edit.png" height="16" width="16" alt="ico" /><div class="tt"><span class="one">Editar</span><span class="two"></span></div></a>';
					tr += '<a href="#" onclick="wConfirm(\'Excluir Registro\',\'Tem certeza que deseja excluir o registro ' + value.nome + ' ?\',\'php\', \'diferenciais_script.php?opx=deletaDiferenciais&idu=' + value.iddiferenciais + '\');"><img src="images/ico_del.png" height="17" width="17" alt="ico" /><div class="tt"><span class="one">Excluir</span><span class="two"></span></div></a>';
					tr += '</div></td>';
				});
...
function verificaUrlrewrite(url) {
	id = $("#iddiferenciais").val();

	$.ajax({
		url: 'diferenciais_script.php',
		dataType: 'json',
		data: "opx=verificarUrlRewrite&iddiferenciais=" + id + "&urlrewrite=" + url,
		type: 'post',
		beforeSend: function () {
			Loader.show();
		},
...
function carregaIconeAcao() {
   $('#icone_pai').on("click", ".icone_icone", function (e) {
      ...
      imgRec = imgRec.replace('files/caracteristicas/', '');
...
function subirFaq ( el ) {
   ...
   var prevTr = $(thisTr).prevAll('.box-diferenciais_faq').first();
...
function descerFaq( el ) {
   ...
   var auxDiv = $('.div-aux');
   var thisTr = $(el).parent().parent().parent();
   var nextTr = $(thisTr).nextAll('.box-diferenciais_faq').first();
   var thisInputPos = $(thisTr).find('.td-flex').find('input');
...
function excluirFaq (el) {
   ...
   $('.removeDiferenciais_faq-'+key).hide();
   // $('.removeFaq-'+key).next('tr').remove();
   $('#excluirDiferenciais_faq-'+key).val('0');
   // $('.removeFaq-'+key).remove();
};

function addActionFaq() {
   $(".subirDiferenciais_faq").unbind( "click" );
   $(".descerDiferenciais_faq").unbind( "click" );
   $(".excluirDiferenciais_faq").unbind( "click" );

   $(".subirDiferenciais_faq").on("click", function () {
       subirFaq($(this));
   });

   $(".descerDiferenciais_faq").on("click", function () {
       descerFaq($(this));
   });

   $(".excluirDiferenciais_faq").on("click", function () {
       excluirFaq($(this));
   });
}
...
$(document).ready(function () {
       ...
       $(".btn-diferenciais_faq").on('click', function () {
         addGrid('diferenciais_faq', false, "pergunta", "resposta");
     });
    ...
    $('.bt_save').click(function(){
        if(imgRec != ''){
            excluirImagemOld('caracteristicas', imgRec);
        }
    });
    ...
    $('.botaoArquivo2').click(function(){
      $('#icone_upload2').click();
    });

   $('.botaoArquivo3').click(function(){
      $('#icone_upload3').click();
   });
    ...
    $("#icone_upload2").change(function(){
      ...
      $("#icone2").show();
      $("#imagem_value2").val($(this)[0].files[0].name);
      $("#inputArquivoBotao2").find('.btn').val($(this)[0].files[0].name);
  });

  $("#icone_upload3").change(function(){
   var preview = $("#icone3");
   var file = $(this)[0].files[0];
   var reader = new FileReader();
   ...
   $("#icone3").show();
   $("#imagem_value3").val($(this)[0].files[0].name);
   $("#inputArquivoBotao3").find('.btn').val($(this)[0].files[0].name);
});
    ...
    $(".excluir-arquivo").click(function(e){
      ...
      $.ajax({
         ...
         type: "POST",
         dataType: "json",
         data: {opx: 'excluirArquivo', iddiferenciais: $("#iddiferenciais").val(), arquivo: $(thisElement).attr('data-arquivo')},
         beforeSend: function () {
            Loader.show();
         },
         success: function (data) {
   ...
   $("#formDiferenciais").on("click", ".btn-choose-icon", function(){
   ...
   //===== Grid itens =====//
      $(".btn-caracteristicas").on('click', function () {
         addGrid('caracteristicas', false);
      });

      $(".btn-utilizadores").on('click', function () {
         addGrid('utilizadores', false);
      });

      $(".btn-funciona").on('click', function () {
        addGrid('funciona', false, false);
      });
      ...
      $(document).on('change', ".file-upload", function(){
        ...
        $(document).on("click", ".subirCaracteristicas", function () {
            var thisDataKey = $(this).parent().parent().parent().attr('data-key');
            var auxDiv = $('.div-aux');
            var thisTr = $(this).parent().parent().parent();
            var prevTr = $(thisTr).prevAll('.box-caracteristicas').first();
            var thisInputPos = $(thisTr).find('.td-flex').find('input');
            var prevInputPos = $(prevTr).find('.td-flex').find('input');
            var auxInputPos = $(thisInputPos).val();
            if($(prevTr).length > 0){
        ...
        $(document).on("click", ".descerCaracteristicas", function () {
            var thisDataKey = $(this).parent().parent().parent().attr('data-key');
            var auxDiv = $('.div-aux');
            var thisTr = $(this).parent().parent().parent();
            var nextTr = $(thisTr).nextAll('.box-caracteristicas').first();
            var thisInputPos = $(thisTr).find('.td-flex').find('input');
            var nextInputPos = $(nextTr).find('.td-flex').find('input');
            var auxInputPos = $(thisInputPos).val();
            if($(nextTr).length > 0){
        ...
        $(document).on("click", ".excluirCaracteristicas", function () {
            var key = $(this).parent().parent().parent().attr('data-key');
            $('.removeCaracteristicas-'+key).hide();
            // $('.removeCaracteristicas-'+key).next('tr').remove();
            $('#excluirCaracteristicas-'+key).val('0');
            // $('.removeCaracteristicas-'+key).remove();
        });

        $(document).on("click", ".subirUtilizadores", function () {
            var thisDataKey = $(this).parent().parent().parent().attr('data-key');
            var auxDiv = $('.div-aux');
            var thisTr = $(this).parent().parent().parent();
            var prevTr = $(thisTr).prevAll('.box-utilizadores').first();
            var thisInputPos = $(thisTr).find('.td-flex').find('input');
            var prevInputPos = $(prevTr).find('.td-flex').find('input');
            var auxInputPos = $(thisInputPos).val();
            if($(prevTr).length > 0){
        ...
        $(document).on("click", ".descerUtilizadores", function () {
            var thisDataKey = $(this).parent().parent().parent().attr('data-key');
            var auxDiv = $('.div-aux');
            var thisTr = $(this).parent().parent().parent();
            var nextTr = $(thisTr).nextAll('.box-utilizadores').first();
            var thisInputPos = $(thisTr).find('.td-flex').find('input');
            var nextInputPos = $(nextTr).find('.td-flex').find('input');
            var auxInputPos = $(thisInputPos).val();
            if($(nextTr).length > 0){
        ...
        $(document).on("click", ".excluirUtilizadores", function () {
            var key = $(this).parent().parent().parent().attr('data-key');
            $('.removeUtilizadores-'+key).hide();
            // $('.removeUtilizadores-'+key).next('tr').remove();
            $('#excluirUtilizadores-'+key).val('0');
            // $('.removeUtilizadores-'+key).remove();
        });

        $(document).on("click", ".subirFunciona", function () {
           var thisDataKey = $(this).parent().parent().parent().attr('data-key');
           var auxDiv = $('.div-aux');
           var thisTr = $(this).parent().parent().parent();
           var prevTr = $(thisTr).prevAll('.box-funciona').first();
           var thisInputPos = $(thisTr).find('.td-flex').find('input');
           var prevInputPos = $(prevTr).find('.td-flex').find('input');
           var auxInputPos = $(thisInputPos).val();
           if($(prevTr).length > 0){
       ...
       $(document).on("click", ".descerFunciona", function () {
           var thisDataKey = $(this).parent().parent().parent().attr('data-key');
           var auxDiv = $('.div-aux');
           var thisTr = $(this).parent().parent().parent();
           var nextTr = $(thisTr).nextAll('.box-funciona').first();
           var thisInputPos = $(thisTr).find('.td-flex').find('input');
           var nextInputPos = $(nextTr).find('.td-flex').find('input');
           var auxInputPos = $(thisInputPos).val();
           if($(nextTr).length > 0){
       ...
       $(document).on("click", ".excluirFunciona", function () {
           var key = $(this).parent().parent().parent().attr('data-key');
           $('.removeFunciona-'+key).hide();
           // $('.removeFunciona-'+key).next('tr').remove();
           $('#excluirFunciona-'+key).val('0');
           // $('.removeFunciona-'+key).remove();
       });
    //===== Fim grid itens =====//
   ...
   ////////////////////////////////////////////
      ...
      $(".btExcluirImagem").click(function(e){
         ...
         iddiferenciais = $("#formDiferenciais").find("#iddiferenciais").val();
         iddiferenciais_imagem = $("#"+idPosicao).find("input[name='iddiferenciais_imagem[]']").val();
         imagem = $("#"+idPosicao).find("input[name='imagem_diferenciais[]']").val();
         ref = $("#"+idPosicao); 
         ...
         $.ajax({
            ...
            data:
            {
              ...
              iddiferenciais:iddiferenciais,            
              imagem:imagem,
              iddiferenciais_imagem:iddiferenciais_imagem
              // descricao: post2
            },
            beforeSend:function(){
              Loader.show();
   ...
   ////////////////////////////////////////////////
    ...
    $('.save-cropped-image').click(function(e){
        ...
        formData.append("iddiferenciais", $('input[name=iddiferenciais]').val());
        if(dataImageType == 'imagem'){
            ...
            success:function(data){
                ...
                if(data.status == true){
                    if(dataImageType == 'imagem'){
                        ...
                        $('#select-image-1 .img-diferenciais-form').attr('src', 'files/diferenciais/'+data.imagem);
                        ...
                        $('#select-image-2 .img-diferenciais-form').attr('src', 'files/diferenciais/'+data.imagem);
    ...
    //         location.href='index.php?mod=diferenciais&acao=listarDiferenciais';
    //     }
    //     else{
    //         if($("#iddiferenciais").val() == 0){
    //             if($("#imagem-value").val() != ''){
    //                 cancelarImagem('diferenciais', $("#imagem-value").val());
    //             }
    ...
    $(".confirm").click(function(){
        excluirImagem('diferenciais', tipo, img, thisElem);
...
function enviaImagensAjax(input, posicao, limite){ 
  ...
  formData.append('iddiferenciais', $("#iddiferenciais").val()); 
  ...
  $.ajax({ 
    ...
    success:function(data){  
        if(data.status == true){ 
            $li = '<li class="ui-state-default'+posicao+' move" id="'+posicao+'" idimagem="'+data.iddiferenciais_imagem+'">';
            $li += '<img id="img'+posicao+'" class="imagem-gallery" src="'+data.caminho+'">';
            $li += '<a class="editImagemDescricao" idimagem="'+data.iddiferenciais_imagem+'" href="#"><button class="edit"></button></a>';
            $li += '<a class="excluirImagem" idimagemdelete="'+data.iddiferenciais_imagem+'" href="#"><button class="delete"></button></a>';
            // $li += '<a class="postImagem" idimagempost="'+data.iddiferenciais_imagem+'" href="#"><button class="post_imagem"></button></a>'; 
            // $li += '<a class="postImagem" idimagempost="'+data.iddiferenciais_imagem+'" href="#"><button class="post_imagem"></button></a>';
            $li += '<input type="hidden" name="iddiferenciais_imagem[]" value="'+data.iddiferenciais_imagem+'">'; 
            $li += '<input type="hidden" name="descricao_imagem[]" value="">'; 
            $li += '<input type="hidden" name="imagem_diferenciais[]" value="'+data.nome_arquivo+'">';
            $li += '<input type="hidden" name="posicao_imagem[]" value="'+posicao+'">';
            $li += '</li>'; 
            $("#sortable").append($li); 
            $("#iddiferenciais").val(data.iddiferenciais); 
            if(numImagem > 1){
...
function resetOrdemImagens(){
  ...
  if($("#mod").val() == "editar"){  
    ...
    form = $("#formDiferenciais").serialize(); 
