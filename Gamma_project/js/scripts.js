$(document).ready(function(){
   ...
   $("#enviar-contato").on("click",function(e){
     ...
     enviarFormEmail("contato",true);
      ...
      //Formulários
      $("#enviar-cadastro-orcamento").on("click",function(e){
        e.preventDefault();
        enviarFormEmail("cadastro-orcamento",true);
      });

   $("#enviar-revendedor").on("click",function(e){
    e.preventDefault();
    enviarFormEmail("revendedor",true);
  });

    const filtrarBtn = document.querySelector('.filtrarBtn');

        filtrarBtn.addEventListener('click', function() {
            // Array para armazenar as marcas selecionadas
            ...
            checkboxes.forEach(function(checkbox) {
                if (checkbox.checked) {
                    // Se o checkbox estiver marcado, adicione a URL correspondente ao array
                    marcasSelecionadas.push(checkbox.getAttribute('data-url'));
                }
            });

            // Criar a URL com as marcas selecionadas
            const urlMarcas = marcasSelecionadas.join('+'); // Ou use outro separador desejado

            // Redirecionar para a página com as marcas selecionadas na URL
            window.location.href = 'produtos/marcas/' + urlMarcas;
        });
   //Trabalhe conosco
   var arquivo;
   ...
   $("#enviar-trabalhe-conosco").on("click", function(e){
    ...
    $('.adicionar').click(function() {
        // Realizar uma requisição AJAX para atualizar a quantidade no servidor
        $.ajax({
            type: 'POST',
            url: 'atualizar_quantidade.php', // Substitua com o nome do seu script PHP
            data: { operacao: 'adicionar' }, // Envie o tipo de operação para o servidor
            success: function(response) {
                // Atualizar a interface do usuário com base na resposta do servidor
                $('#qtd').val(response); // Atualiza o valor exibido no campo de quantidade
            }
        });
    });
    ...
    $('.remover').click(function() {
        ...
        $.ajax({
            type: 'POST',
            url: 'atualizar_quantidade.php', // Substitua com o nome do seu script PHP
            data: { operacao: 'remover' }, // Envie o tipo de operação para o servidor
            success: function(response) {
                // Atualizar a interface do usuário com base na resposta do servidor
                $('#qtd').val(response); // Atualiza o valor exibido no campo de quantidade
            }
