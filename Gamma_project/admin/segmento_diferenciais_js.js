...
function preTableSegmento_diferenciais(){
		 $("#limit").change(function(){
	         $("#pagina").val(1);
	         dataTableSegmento_diferenciais();
	    });
      ...
      $('.table').on("click",".ordemUp",function(e){ 
          var params = { idsegmento_diferenciais: $(this).attr("codigo")}
          $.post( 'segmento_diferenciais_script.php?opx=alteraOrdemCima', params,
                function(data){  
    ...
    $('.table').on("click", ".ordemDown", function(e){  
        var params = {idsegmento_diferenciais: $(this).attr("codigo")}  
        $.post( 'segmento_diferenciais_script.php?opx=alteraOrdemBaixo', params,
         function(data){  
            var resultado = new String (data.status);  
            if(resultado.toString() == 'sucesso'){ 
              dataTableSegmento_diferenciais(); 
...
var myColumnDefs = [
	{key:"idsegmento_diferenciais", sortable:true, label:"ID", print:true, data:true},
...
function columnSegmento_diferenciais(){ 
    tr = "";
...
function dataTableSegmento_diferenciais(){
    limit = $("#limit").val();
    pagina = $("#pagina").val();
          ...
          success:function(data){
                ...
                if(data.totalRecords > 0){
                    ...
                    $.each(myColumnDefs, function(col, ColumnDefs){
                      ...
                      tr += '<a href="index.php?mod=segmento_diferenciais&acao=formSegmento_diferenciais&met=editSegmento_diferenciais&idu='+value.idsegmento_diferenciais+'"><img src="images/ico_edit.png" height="16" width="16" alt="ico" /><div class="tt"><span class="one">Editar</span><span class="two"></span></div></a>';
                      tr += '<a href="#" onclick="wConfirm(\'Excluir Registro\',\'Tem certeza que deseja excluir o registro '+value.idsegmento_diferenciais+' ?\',\'php\', \'segmento_diferenciais_script.php?opx=deletaSegmento_diferenciais&idu='+value.idsegmento_diferenciais+'\');"><img src="images/ico_del.png" height="17" width="17" alt="ico" /><div class="tt"><span class="one">Excluir</span><span class="two"></span></div></a>';
                      tr += '</div></td>';
                    }); 
...
$(document).ready(function(){
    $(".cancelar").click(function(event){
        location.href='index.php?mod=segmento_diferenciais&acao=listarSegmento_diferenciais';
    });
    ...
    $(".confirm").click(function(){
        excluirImagem('segmento_diferenciais', tipo, img, thisElem);
