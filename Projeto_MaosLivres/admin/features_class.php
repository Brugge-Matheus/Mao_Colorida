<?php 
	...
	function cadastroFeatures($dados)
	{
      ...
      $dados['sufixo'] = trim($dados['sufixo']);
		...
		$sql = "INSERT INTO features(icone, titulo, ordem, prefixo, sufixo, numero, icone_name, home, imagem, status) VALUES (
						'".$dados['icone']."',
						'".$dados['titulo']."',
                        '".$dados['ordem']."',
                        '".$dados['prefixo']."',
	...
	function buscaFeatures($dados = array())
	{
      ...
      if (array_key_exists('home',$dados) && !empty($dados['home']) )
		...
		$sql = "SELECT $colsSql, C.home as home_nome $buscaMax FROM features as C 
				WHERE 1  $buscaId $buscaHome $buscaNome $buscaOrdem  $buscaStatus $orderBy $buscaLimit ";
		...
		while ($r = mysqli_fetch_assoc($query)){
			...
			if (!array_key_exists('totalRecords',$dados)){
						...
						$r["status_icone"] = "<img src='images/estrela".($r["status"]== 1 ? "sim":"nao").".png' class='icone inverteStatus' codigo='".$r['idfeatures']."' width='20px' />";
