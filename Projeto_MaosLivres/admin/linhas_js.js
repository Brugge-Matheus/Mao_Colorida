...
function preTableLinhas() {
   ...
   $('.table').on("click", ".inverteStatus", function (e) {
      var params = {
         idlinhas: $(this).attr("codigo")
      }

      $.post(
         'linhas_script.php?opx=inverteStatus',
         params,
         function (data) {
...
var myColumnDefs = [
	{ key: "idlinhas", sortable: true, label: "ID", print: true, data: true },
		...
		success: function (data) {
			...
			if (data.totalRecords > 0) {
				$.each(data.records, function (index, value) {
					...
					tr += '<a href="index.php?mod=linhas&acao=formLinhas&met=editLinhas&idu=' + value.idlinhas + '"><img src="images/ico_edit.png" height="16" width="16" alt="ico" /><div class="tt"><span class="one">Editar</span><span class="two"></span></div></a>';
					tr += '<a href="#" onclick="wConfirm(\'Excluir Registro\',\'Tem certeza que deseja excluir o registro ' + value.nome + ' ?\',\'php\', \'linhas_script.php?opx=deletaLinhas&idu=' + value.idlinhas + '\');"><img src="images/ico_del.png" height="17" width="17" alt="ico" /><div class="tt"><span class="one">Excluir</span><span class="two"></span></div></a>';
					tr += '</div></td>';
				});
...
function verificaUrlrewrite(url) {
	...
	$.ajax({
		url: 'linhas_script.php',
		dataType: 'json',
		data: "opx=verificarUrlRewrite&idlinhas=" + id + "&urlrewrite=" + url,
		type: 'post',
		beforeSend: function () {
			Loader.show();
		},
...
function subirFaq ( el ) {
   ...
   var prevTr = $(thisTr).prevAll('.box-linhas_faq').first();
...
function descerFaq( el ) {
   ...
   var auxDiv = $('.div-aux');
   var thisTr = $(el).parent().parent().parent();
   var nextTr = $(thisTr).nextAll('.box-linhas_faq').first();
   var thisInputPos = $(thisTr).find('.td-flex').find('input');
...
function excluirFaq (el) {
   ...
   $('.removeLinhas_faq-'+key).hide();
   // $('.removeFaq-'+key).next('tr').remove();
   $('#excluirLinhas_faq-'+key).val('0');
   // $('.removeFaq-'+key).remove();
};

function addActionFaq() {
   $(".subirLinhas_faq").unbind( "click" );
   $(".descerLinhas_faq").unbind( "click" );
   $(".excluirLinhas_faq").unbind( "click" );

   $(".subirLinhas_faq").on("click", function () {
       subirFaq($(this));
   });

   $(".descerLinhas_faq").on("click", function () {
       descerFaq($(this));
   });

   $(".excluirLinhas_faq").on("click", function () {
       excluirFaq($(this));
   });
}
...
$(document).ready(function () {
       ...
       $(".btn-linhas_faq").on('click', function () {
         addGrid('linhas_faq', false, "pergunta", "resposta");
     });
  ...
  $("#icone_upload3").change(function(){
   ...
   $("#inputArquivoBotao3").find('.btn').val($(this)[0].files[0].name);
    ...
    $(".excluir-arquivo").click(function(e){
      ...
      $.ajax({
         ...
         type: "POST",
         dataType: "json",
         data: {opx: 'excluirArquivo', idlinhas: $("#idlinhas").val(), arquivo: $(thisElement).attr('data-arquivo')},
         beforeSend: function () {
            Loader.show();
         },
         success: function (data) {
   ...
   $("#formLinhas").on("click", ".btn-choose-icon", function(){
   ...
   ////////////////////////////////////////////
      ...
      $(".btExcluirImagem").click(function(e){
         ...
         idlinhas = $("#formLinhas").find("#idlinhas").val();
         idlinhas_imagem = $("#"+idPosicao).find("input[name='idlinhas_imagem[]']").val();
         imagem = $("#"+idPosicao).find("input[name='imagem_linhas[]']").val();
         ref = $("#"+idPosicao); 
         ...
         $.ajax({
            ...
            data:
            {
              ...
              idlinhas:idlinhas,            
              imagem:imagem,
              idlinhas_imagem:idlinhas_imagem
              // descricao: post2
            },
            beforeSend:function(){
              Loader.show();
   ...
   ////////////////////////////////////////////////
    ...
    $('.save-cropped-image').click(function(e){
        ...
        formData.append("idlinhas", $('input[name=idlinhas]').val());
        if(dataImageType == 'imagem'){
            ...
            success:function(data){
                ...
                if(data.status == true){
                    if(dataImageType == 'imagem'){
                        ...
                        $('#select-image-1 .img-linhas-form').attr('src', 'files/linhas/'+data.imagem);
                        ...
                        $('#select-image-2 .img-linhas-form').attr('src', 'files/linhas/'+data.imagem);
    ...
    //         location.href='index.php?mod=linhas&acao=listarLinhas';
    //     }
    //     else{
    //         if($("#idlinhas").val() == 0){
    //             if($("#imagem-value").val() != ''){
    //                 cancelarImagem('linhas', $("#imagem-value").val());
    //             }
    ...
    $(".confirm").click(function(){
        excluirImagem('linhas', tipo, img, thisElem);
...
function enviaImagensAjax(input, posicao, limite){ 
  ...
  formData.append('idlinhas', $("#idlinhas").val()); 
  ...
  $.ajax({ 
    ...
    success:function(data){  
        if(data.status == true){ 
            $li = '<li class="ui-state-default'+posicao+' move" id="'+posicao+'" idimagem="'+data.idlinhas_imagem+'">';
            $li += '<img id="img'+posicao+'" class="imagem-gallery" src="'+data.caminho+'">';
            $li += '<a class="editImagemDescricao" idimagem="'+data.idlinhas_imagem+'" href="#"><button class="edit"></button></a>';
            $li += '<a class="excluirImagem" idimagemdelete="'+data.idlinhas_imagem+'" href="#"><button class="delete"></button></a>';
            // $li += '<a class="postImagem" idimagempost="'+data.idlinhas_imagem+'" href="#"><button class="post_imagem"></button></a>'; 
            // $li += '<a class="postImagem" idimagempost="'+data.idlinhas_imagem+'" href="#"><button class="post_imagem"></button></a>';
            $li += '<input type="hidden" name="idlinhas_imagem[]" value="'+data.idlinhas_imagem+'">'; 
            $li += '<input type="hidden" name="descricao_imagem[]" value="">'; 
            $li += '<input type="hidden" name="imagem_linhas[]" value="'+data.nome_arquivo+'">';
            $li += '<input type="hidden" name="posicao_imagem[]" value="'+posicao+'">';
            $li += '</li>'; 
            $("#sortable").append($li); 
            $("#idlinhas").val(data.idlinhas); 
            if(numImagem > 1){
