...
function preTableTratamentos() {
	$("#limit").change(function () {
		$("#pagina").val(1);
		dataTableTratamentos();
	});
   ...
   $('.table').on("click", ".inverteStatus", function (e) {
      var params = {
         idtratamentos: $(this).attr("codigo")
      }

      $.post(
         'tratamentos_script.php?opx=inverteStatus',
         params,
         function (data) {
...
var myColumnDefs = [
	{ key: "idtratamentos", sortable: true, label: "ID", print: true, data: true },
		...
		success: function (data) {
			...
			if (data.totalRecords > 0) {
				$.each(data.records, function (index, value) {
					...
					tr += '<a href="index.php?mod=tratamentos&acao=formTratamentos&met=editTratamentos&idu=' + value.idtratamentos + '"><img src="images/ico_edit.png" height="16" width="16" alt="ico" /><div class="tt"><span class="one">Editar</span><span class="two"></span></div></a>';
					tr += '<a href="#" onclick="wConfirm(\'Excluir Registro\',\'Tem certeza que deseja excluir o registro ' + value.nome + ' ?\',\'php\', \'tratamentos_script.php?opx=deletaTratamentos&idu=' + value.idtratamentos + '\');"><img src="images/ico_del.png" height="17" width="17" alt="ico" /><div class="tt"><span class="one">Excluir</span><span class="two"></span></div></a>';
					tr += '</div></td>';
				});
...
function verificaUrlrewrite(url) {
	id = $("#idtratamentos").val();

	$.ajax({
		url: 'tratamentos_script.php',
		dataType: 'json',
		data: "opx=verificarUrlRewrite&idtratamentos=" + id + "&urlrewrite=" + url,
		type: 'post',
		beforeSend: function () {
			Loader.show();
		},
...
function carregaIconeAcao() {
   $('#icone_pai').on("click", ".icone_icone", function (e) {
      ...
      imgRec = imgRec.replace('files/recursos/', '');
...
$(document).ready(function () {
    ...
    $('.bt_save').click(function(){
        if(imgRec != ''){
            excluirImagemOld('recursos', imgRec);
        }
    });
    ...
    $(".excluir-arquivo").click(function(e){
      ...
      $.ajax({
         ...
         type: "POST",
         dataType: "json",
         data: {opx: 'excluirArquivo', idtratamentos: $("#idtratamentos").val(), arquivo: $(thisElement).attr('data-arquivo')},
         beforeSend: function () {
            Loader.show();
         },
         success: function (data) {
   ...
   $("#formTratamentos").on("click", ".btn-choose-icon", function(){
   ...
   //===== Grid itens =====//
      $(".btn-recursos").on('click', function () {
         addGrid('recursos', true);
      });
      ...
      // });
        ...
        $(document).on("click", ".descerRecursos", function () {
            var thisDataKey = $(this).parent().parent().parent().attr('data-key');
            var auxDiv = $('.div-aux');
            var thisTr = $(this).parent().parent().parent();
            var nextTr = $(thisTr).nextAll('.box-recursos').first();
            var thisInputPos = $(thisTr).find('.td-flex').find('input');
        ...
        $(document).on("click", ".excluirRecursos", function () {
            ...
            // $('.removeRecursos-'+key).next('tr').remove();
   ...
   ////////////////////////////////////////////
      ...
      $(".btExcluirImagem").click(function(e){
         ...
         idtratamentos = $("#formTratamentos").find("#idtratamentos").val();
         idtratamentos_imagem = $("#"+idPosicao).find("input[name='idtratamentos_imagem[]']").val();
         imagem = $("#"+idPosicao).find("input[name='imagem_tratamentos[]']").val();
         ref = $("#"+idPosicao); 
         ...
         $.ajax({
            ...
            data:
            {
              ...
              idtratamentos:idtratamentos,            
              imagem:imagem,
              idtratamentos_imagem:idtratamentos_imagem
              // descricao: post2
            },
            beforeSend:function(){
              Loader.show();
   ...
   ////////////////////////////////////////////////
    ...
    $('.save-cropped-image').click(function(e){
        ...
        formData.append("idtratamentos", $('input[name=idtratamentos]').val());
        if(dataImageType == 'imagem'){
            ...
            success:function(data){
                ...
                if(data.status == true){
                    if(dataImageType == 'imagem'){
                        ...
                        $('#select-image-1 .img-tratamentos-form').attr('src', 'files/tratamentos/'+data.imagem);
                        ...
                        $('#select-image-2 .img-tratamentos-form').attr('src', 'files/tratamentos/'+data.imagem);
    ...
    $(".cancelar").click(function(event){
        ...
        if($("#imagem-value").val() == '' && $("#imagem_2-value").val() == ''){
            location.href='index.php?mod=tratamentos&acao=listarTratamentos';
        }
        else{
            if($("#idtratamentos").val() == 0){
                if($("#imagem-value").val() != ''){
                    cancelarImagem('tratamentos', $("#imagem-value").val());
                }
    ...
    $(".confirm").click(function(){
        excluirImagem('tratamentos', tipo, img, thisElem);
...
function enviaImagensAjax(input, posicao, limite){ 
  ...
  formData.append('idtratamentos', $("#idtratamentos").val()); 
  ...
  $.ajax({ 
    ...
    success:function(data){  
        if(data.status == true){ 
            $li = '<li class="ui-state-default'+posicao+' move" id="'+posicao+'" idimagem="'+data.idtratamentos_imagem+'">';
            $li += '<img id="img'+posicao+'" class="imagem-gallery" src="'+data.caminho+'">';
            $li += '<a class="editImagemDescricao" idimagem="'+data.idtratamentos_imagem+'" href="#"><button class="edit"></button></a>';
            $li += '<a class="excluirImagem" idimagemdelete="'+data.idtratamentos_imagem+'" href="#"><button class="delete"></button></a>';
            // $li += '<a class="postImagem" idimagempost="'+data.idtratamentos_imagem+'" href="#"><button class="post_imagem"></button></a>'; 
            // $li += '<a class="postImagem" idimagempost="'+data.idtratamentos_imagem+'" href="#"><button class="post_imagem"></button></a>';
            $li += '<input type="hidden" name="idtratamentos_imagem[]" value="'+data.idtratamentos_imagem+'">'; 
            $li += '<input type="hidden" name="descricao_imagem[]" value="">'; 
            $li += '<input type="hidden" name="imagem_tratamentos[]" value="'+data.nome_arquivo+'">';
            $li += '<input type="hidden" name="posicao_imagem[]" value="'+posicao+'">';
            $li += '</li>'; 
            $("#sortable").append($li); 
            $("#idtratamentos").val(data.idtratamentos); 
            if(numImagem > 1){
...
function resetOrdemImagens(){
  ...
  if($("#mod").val() == "editar"){  
    ...
    form = $("#formTratamentos").serialize(); 
