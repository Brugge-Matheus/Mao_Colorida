...
function preTableFaq(){
	...
	$('.table').on("click",".ordemUp",function(e){
		var params = {
			idfaq: $(this).attr("codigo") 
		}

		$.post(
			'faq_script.php?opx=alteraOrdemCima',
			params,
			function(data){
	...
	$('.table').on("click", ".ordemDown", function(e){
		...
		$.post(
			'faq_script.php?opx=alteraOrdemBaixo',
			params,
			function(data){
				var resultado = new String (data.status);
	...
	$('.table').on("click", ".inverteStatus", function (e) {
		...
		$.post(
			'faq_script.php?opx=inverteStatus',
			params,
			function (data) {
				var resultado = new String(data.status);
...
var myColumnDefs = [
	{key:"idfaq", sortable:true, label:"ID", print:true, data:true},
			...
			success:function(data){
				...
				if(data.totalRecords > 0){
					$.each(data.records, function(index, value){
						...
						tr += '<a href="index.php?mod=faq&acao=formFaq&met=editFaq&idu='+value.idfaq+'"><img src="images/ico_edit.png" height="16" width="16" alt="ico" /><div class="tt"><span class="one">Editar</span><span class="two"></span></div></a>';
						tr += '<a href="#" onclick="wConfirm(\'Excluir Registro\',\'Tem certeza que deseja excluir o registro '+value.idfaq+' ?\',\'php\', \'faq_script.php?opx=deletaFaq&idu='+value.idfaq+'\');"><img src="images/ico_del.png" height="17" width="17" alt="ico" /><div class="tt"><span class="one">Excluir</span><span class="two"></span></div></a>';
						tr += '</div></td>';
					});
...
$(document).ready(function(){
    $(".cancelar").click(function(event){
        location.href='index.php?mod=faq&acao=listarFaq';
    });
});